...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($lastName) || empty($firstName) || empty($email) || empty($password) || empty($confirmPassword) || empty($studentId) || empty($course) || empty($yearLevel)) {
        ...
        header('Location: ../your_error_page.php'); // Change to your error page
        exit();
    }
    ...
    try {
        $sql = "INSERT INTO students (fullName, student_id, course, year_level, email, password, qr_code) 
                VALUES (:fullName, :student_id, :course, :year_level, :email, :password, :qr_code)";
