...
function getCount()
{
    ...
    $stmt->execute([
        ':userId' => $userId,
        ':userType' => 'staff'  // Adjust this based on the logged-in user type
    ]);
...
function searchUsers()
{
    ...
    $results = [];
    $tables = [
        'admin' => 'CONCAT(first_name, " ", last_name) AS name, id, "admin" AS type, first_name, last_name',  // Add first_name and last_name here for WHERE clause
        'students' => 'fullName AS name, id, "student" AS type, fullName',  // Add fullName for WHERE clause
        'staff_accounts' => 'fullName AS name, id, "staff" AS type, fullName',  // Add fullName for WHERE clause
    ];

    foreach ($tables as $table => $fields) {
        // Update WHERE clause to use actual columns for filtering
        if ($table == 'admin') {
            $stmt = $pdo->prepare("SELECT $fields FROM $table WHERE CONCAT(first_name, ' ', last_name) LIKE :query");
        } else {
            $stmt = $pdo->prepare("SELECT $fields FROM $table WHERE fullName LIKE :query");
        }
