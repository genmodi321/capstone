        ...
        <div class="container-fluid actual-content">
          <div class="container wd-75">
            ...
            <div class="container-fluid">
              <h5>Here are the classes you are currently handling:</h5>
              <br>
              <?php
                            $teacher_name = $_SESSION['teacher_name']; // Teacher's ID from session
              $sql = "SELECT * FROM classes WHERE teacher = :teacher_full_name";
              $stmt = $pdo->prepare($sql);
              $stmt->bindParam(':teacher_full_name', $_SESSION['teacher_name']);
              $stmt->execute();
              $classes = $stmt->fetchAll(PDO::FETCH_ASSOC);
              if (empty($classes)) {
                echo "<div class='row text-center d-flex justify-content-center align-content-center'>
                ...
                foreach ($classes as $class) {
                  echo "<div class='row text-center d-flex justify-content-center align-content-center'>
                    <div class='col-md-3 class-container'>
                        <p><i class='bi bi-calendar'></i> <b>Class:</b> {$class['name']}</p>
                        <p><i class='bi bi-journal-album'></i> <b>Subject:</b> {$class['subject']}</p>
                        <p><i class='bi bi-file-person'></i> <b>Students:</b> {$class['studentTotal']}</p>
                           <button class='btn btn-csms-outlined' onclick='gotoSubject({$class['id']})'>Open</button>
                    </div>
                  </div>";
                }
              }
...
<script>
  ...
  function gotoSubject(id) {
    window.location.href = "teacher_class_management.php?id=" + id;
  }
</script>
